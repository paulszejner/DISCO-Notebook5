...
```
In these steps, you can see that the various temporal resolutions yield different values of d18O. This result from variability of environmental conditions throughout (whole growing season) or within (window)  the growing season  captured in d18O measurements of annual, EW-LW and subseasonal resolution/tree-ring sections.
...
par(mar=c(4,4,2,0),lwd=0.5)
layout(matrix(1:2,nrow = 1, ncol = 2,byrow = T), widths = c(10,10),heights = c(10,10),respect = TRUE)
#VPD_1998
...
par(mar=c(4,4,2,0),lwd=0.5)
plot(HEL_98[,1], HEL_98[,2], type="n", ylim=c(26,34), xlim=c(135,304), axes=F, xlab = "DOY", ylab = "d18O (permill.VSMOW)");box()
axis(1, at = seq(135,304,15),lwd=0.5,las=2)
axis(2,lwd=0.5,las=2)
grid(lwd=1)
lines( HEL_98[,1],HEL_98[,2], type="o",col=colb1[2],lwd=0.5, pch=1)
legend(x = 210, y = 34, legend = "Thin sections", col=colb1[3] , bty = "n",lwd=1)
# add the values of the quarters (equally distanced thin sections)
lines( HEL_98[,1],HEL_98_quarter, type="S",col=colb1[3],lwd=3)
legend(x = 210, y = 33.5, legend = "4 sections", col=colb1[3] , bty = "n",lwd=3)
# Add the values EW and LW 
lines(HEL_98[,1][1:11], rep(EW_1998, 11),type="s",col="darkblue",lwd=3  )#EW
legend(x = 210, y = 32.5, legend = "Earlywood", col="darkblue" , bty = "n",lwd=3)
lines(HEL_98[,1][12:16], rep(LW_1998, 5),type="s",col="darkred",lwd=3  )#LW
legend(x = 210, y = 32, legend = "Latewood", col="darkred" , bty = "n",lwd=3)
# add the annual 18O value
abline(h = HEL_98_complete,col=colb1[1],lwd=3,lty=2)
legend(x = 210, y = 33, legend = "annual mean", col=colb1[1] , bty = "n",lwd=3,lty=2)
...
layout(matrix(1:2,nrow = 1, ncol = 2,byrow = T), widths = c(2,2),heights = c(1.7,1.7),respect = TRUE)
...
Question 8: Describe the temporal trends of each isotope and for each tree ring subdivision (EW & LW)

Nest, we will explore the differences between EW and LW for each isotope
